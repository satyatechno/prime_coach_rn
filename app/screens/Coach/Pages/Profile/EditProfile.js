...
const EditProfile = ({ navigation }) => {
  ...
  useEffect(() => {
    if (userProfile?.id) {
      ...
      if (specialisationTemp) {
        setSpecialization(specialisationTemp?.value);
      } else {
        setSpecialization(userProfile?.specialization ?? null);
      }
  ...
  const checkValidation = () => {
    ...
    if (!firstName?.trim()?.length) {
      ...
      Toaster("Specialization is required.", Colors.LIGHT_RED);
      return false;
  ...
  const updateProfile = async () => {
    ...
    if (isValid) {
      ...
      // formdata.append(
      //   "specialization",
      //   specialization ?? coachSpec?.pickerPlaceholder?.value
      // );
      formdata.append(
        "dob",
      ...
      >
        ...
        <View style={{ marginTop: 10 }}>
          <Select
            ...
            placeholder={
              userProfile?.specialization ?? "Select Coach Specialization"
            }
          />
        </View>
        <CustomInput
